...
- Added `AuthStorage.getOAuthAccountIdentity()`, strict `AuthStorage.getSessionOAuthAccountIdentity()`, and the `OAuthAccountIdentity` type. The general read-only lookup preserves first-account fallback for metadata callers, while the strict lookup returns `accountId`/`email`/`projectId` only for the OAuth credential already routed to the exact session.
