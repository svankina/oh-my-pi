...
describe("ModelSelector search stays inside the active provider tab (#4522)", () => {
	...
	test("compact subscription selection fuzzy-searches only OAuth Codex and Anthropic models", async () => {
		installTestTheme();
		const codex = makeModel("openai-codex", "gpt-5.3-codex");
		const anthropic = makeModel("anthropic", "claude-sonnet-4-5");
		const apiKeyModel = makeModel("openai", "gpt-5.3");
		const models = [codex, anthropic, apiKeyModel];
		const selected: Model[] = [];
		const modelRegistry = {
			getAll: () => models,
			refresh: vi.fn(async () => {}),
			...
			getAvailable: () => models,
			getDiscoverableProviders: () => [],
			isUsingOAuth: (model: Model) => model.provider === "anthropic" || model.provider === "openai-codex",
		} as unknown as ModelRegistry;
		...
		const selector = new ModelSelectorComponent(
			...
			{
				directSelect: true,
				allowedProviders: ["anthropic", "openai-codex"],
				requireOAuth: true,
				compact: true,
			},
		);
		...
		const rendered = normalizeRenderedText(selector.render(220).join("\n"));
		expect(rendered).toContain("Codex and Anthropic subscription models");
		expect(rendered).toContain("anthropic/claude-sonnet-4-5");
		expect(rendered).toContain("openai-codex/gpt-5.3-codex");
		expect(rendered).not.toContain("openai/gpt-5.3");

		for (const ch of "sonnet") {
			selector.handleInput(ch);
		}
		selector.handleInput("\n");

		expect(selected).toEqual([anthropic]);
	});
});
