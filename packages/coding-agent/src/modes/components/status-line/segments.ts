...
const usageSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		const tiers = new Set(
			usage
				.map(limit => (limit.tier ? truncateToWidth(sanitizeStatusText(limit.tier), TRUNCATE_LENGTHS.SHORT) : ""))
				.filter(Boolean),
		);
		const sharedTier = tiers.size === 1 ? tiers.values().next().value : undefined;
		const parts: string[] = sharedTier ? [theme.fg("accent", sharedTier)] : [];
		for (const limit of usage) {
			...
			const tier =
				!sharedTier && limit.tier
					? ` (${truncateToWidth(sanitizeStatusText(limit.tier), TRUNCATE_LENGTHS.SHORT)})`
					: "";
			parts.push(`${sanitizeStatusText(limit.label)}${tier} ${percent}${reset}`);
		}
